...
export function Sidebar({
  ...
  useEffect(() => {
    ...
    const hasCompletionData = modules.some((m) =>
      m.pages.some((p) => p.completed),
    );
  ...
  const groupedModules = modules
    .filter((module) => {
      ...
      return (
        learningPath !== "Walkthroughs" && learningPath == currentLearningPath
      );
    })
    .reduce(
      (acc, module) => {
  ...
  const sidebarContent = (
    <aside
      ref={sidebarRef}
      className="h-full bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 overflow-y-auto overscroll-contain"
    >
                ...
                <div key={learningPath} className="mb-6">
                  ...
                  <ul className="space-y-2">
                    {pathModules.map((module) => {
                      ...
                      const completedPages = module.pages.filter(
                        (p) => p.completed,
                      ).length;
                          ...
                          >
                            ...
                            <div className="flex-shrink-0">
                              {allCompleted ? (
                                ...
                                <div
                                  className="w-5 h-5 rounded-full border-2 border-gray-400 dark:border-gray-600"
                                  aria-hidden="true"
                                />
                              )}
                            </div>
  ...
  return (
    <>
      ...
      {/* Desktop Sidebar */}
      <div
        className={`hidden md:block fixed left-0 top-16 h-[calc(100vh-4rem)] transition-all duration-300 ${isCollapsed ? "w-14" : "w-70"}`}
      >
        {sidebarContent}
      </div>
